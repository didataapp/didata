defmodule DidataWeb.UserSessionControllerTest do
  use DidataWeb.ConnCase, async: true

  import Didata.AccountsFixtures
  ...
  describe "GET /users/login" do
    ...
    test "redirects if already logged in", %{conn: conn, user: user} do
      conn =
        conn |> DidataWeb.ConnCase.login_user(user) |> get(Routes.user_session_path(conn, :new))
  ...
  describe "DELETE /users/logout" do
    ...
    test "logs the user out", %{conn: conn, user: user} do
      conn =
        conn
        |> DidataWeb.ConnCase.login_user(user)
